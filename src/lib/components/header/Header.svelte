<script lang="ts">
  ...
  const navlinks = [
    { route: '/', text: 'Home' },
    //{ route: '/showcase', text: 'Showcase' },
    { route: '/about', text: 'About' },
    { route: '/contact', text: 'Contact' }
  ];

  const sociallinks = [
    { route: 'https://github.com/chrisgbradley', icon: 'mdi:github'},
    { route: 'https://www.linkedin.com/in/christianbradley/', icon: 'mdi:linkedin'},
    { route: 'https://dribbble.com/chrisgbradley', icon: 'mdi:dribbble'},
  ]
</script>
...
<header class="container max-w-7xl mx-auto p-3">
  <nav class="">
    ...
    <div class="controls">
      ...
      <ul
        id="mobile-menu"
        class="fixed w-[300px] h-[100vh] top-0 right-0 text-lg flex flex-col pt-32 items-center z-10 bg-primary lg:pt-0 lg:flex-row lg:flex lg:relative lg:w-auto lg:h-auto lg:bg-opacity-0"
        class:hidden={!isMenuOpen && !smscr}
        class:sidebar-closed={!isMenuOpen && smscr} 
        class:sidebar-open={isMenuOpen && smscr} 
        class:menu-open-slide={isMenuOpen}
      >
        {#each navlinks as link}
          <li
            class="text-center lg:w-24 text-accent active:text-secondary text-xl font-bold lg:text-secondary lg:text-lg lg:font-normal lg:active:text-accent flex justify-center items-center py-8 lg:py-0"
            on:click={() => {
              isMenuOpen = false;
            }}
            class:active={$page.url.pathname === link.route}
          >
            <a sveltekit:prefetch href={link.route}>{link.text}</a>
          </li>
        {/each}
      </ul>
      ...
      <div class=" justify-center items-center gap-3 hidden mb-1 lg:flex">
        {#each sociallinks as link}
          <a href={link.route} target="_blank" class="">
            <Icon icon={link.icon} style="font-size: 1.75rem;" inline={true} class="text-accent" />
          </a>
        {/each}
      </div>

      <button
        class="text-md border-2 rounded-md px-3 lg:mb-2 leading-extra-loose border-accent ml-6 flex justify-center items-center gap-2 hover:bg-accent hover:text-secondary transition-all duration-150 z-20"
        class:theme-btn-fix={isMenuOpen}
        on:click={cycleThemes}
        ><Icon icon="@custom:md:theme" style="font-size: 2.25rem;" inline={true} />
        <span class="hidden lg:block">Change Theme</span></button
      >
    </div>
  </nav>
</header>
