...
import Footer from '../footer/Footer.svelte';
  ...
  export let enableSystem: boolean = false;
  /** Whether to indicate to browsers which color scheme is used (dark or light) for built-in UI like inputs and buttons */
  export let enableColorScheme: boolean = true;
  /** Key used to store theme setting in localStorage */
  export let storageKey: string = 'theme';
  /** List of all available theme names */
  export let themes: string[] = enableSystem ? ['light', 'dark', 'ocean', 'forest', 'system'] : ['light', 'dark', 'ocean', 'forest'];
  ...
  const changeTheme = (theme: string | undefined, updateStorage = true, updateDOM = true) => {
    if (theme === undefined) {
      console.log('changeTheme() is called, but the theme is undefined.');
      return;
    }
